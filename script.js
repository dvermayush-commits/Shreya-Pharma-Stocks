...
// ================= MASTER DATA (v26 - Match PDF Exactly) =================
const exactMasterData = [
    ...
    { id: "M7", name: "CALCIBONE GUMMY", div: "MEDK", pack: "1*30", batch: "ASG-2602", exp: "8/27", hsn: "21069099", oldMrp: 350, mrp: 328, baseRate: 80, rate: 230, igst: 5, qty: 6, free: 0 },
    { id: "M8", name: "DIZIREX-16", div: "MEDK", pack: "10*10", batch: "AT251211", exp: "11/27", hsn: "30049099", oldMrp: 950, mrp: 890, baseRate: 70, rate: 300, igst: 5, qty: 2, free: 0 },
    ...
    { id: "M11", name: "EBAZEST 20 TAB", div: "MEDK", pack: "10X10", batch: "AT260214", exp: "1/28", hsn: "30049099", oldMrp: 1000, mrp: 937, baseRate: 90, rate: 370, igst: 5, qty: 3, free: 0 },
    ...
    { id: "M16", name: "GRISOWIN 500 TAB", div: "MEDK", pack: "10X10", batch: "AT26021", exp: "1/28", hsn: "30049099", oldMrp: 1850, mrp: 1734, baseRate: 540, rate: 1000, igst: 5, qty: 2, free: 0 },
    ...
    { id: "M20", name: "HERFIX 400 ТАВ", div: "MEDK", pack: "10X10", batch: "AT26031", exp: "2/28", hsn: "30049099", oldMrp: 1528, mrp: 1459, baseRate: 260, rate: 700, igst: 5, qty: 2, free: 0 },
    ...
    { id: "M23", name: "KLIACET 10 ТАВ", div: "MEDK", pack: "10X10", batch: "25F-T988", exp: "8/27", hsn: "30049099", oldMrp: 700, mrp: 656, baseRate: 50, rate: 180, igst: 5, qty: 5, free: 0 },
    ...
    { id: "M31", name: "KLINZICEF 200DT TAB", div: "MEDK", pack: "10X10", batch: "T250618", exp: "11/27", hsn: "30049029", oldMrp: 1094.2, mrp: 1044, baseRate: 360, rate: 600, igst: 5, qty: 2, free: 0 },
    ...
    { id: "M33", name: "MOXBEX CV 625 TAB", div: "MEDK", pack: "10X10", batch: "TB26-00", exp: "3/28", hsn: "30049099", oldMrp: 2085.4, mrp: 1967, baseRate: 530, rate: 750, igst: 5, qty: 3, free: 0 },
    { id: "M34", name: "MYCLARIX 250 TAB", div: "MEDK", pack: "10X10", batch: "AT25121", exp: "11/27", hsn: "30049099", oldMrp: 0, mrp: 2570, baseRate: 630, rate: 1100, igst: 5, qty: 2, free: 0 },
    { id: "M35", name: "MYCLARIX 500 TAB", div: "MEDK", pack: "10X10", batch: "AT25121", exp: "11/27", hsn: "48192020", oldMrp: 1200, mrp: 4255, baseRate: 1200, rate: 2050, igst: 5, qty: 2, free: 0 },
    { id: "M36", name: "MYCLARIX DRY SYP", div: "MEDK", pack: "30ML", batch: "DS26007", exp: "2/28", hsn: "30049099", oldMrp: 154.56, mrp: 148, baseRate: 26.5, rate: 80, igst: 5, qty: 10, free: 0 },
    { id: "M37", name: "NORSHE 5 TAB", div: "MEDK", pack: "10X10", batch: "HT-2604", exp: "3/28", hsn: "30049099", oldMrp: 603.6, mrp: 575, baseRate: 70, rate: 360, igst: 5, qty: 2, free: 0 },
    ...
    { id: "M41", name: "RANTIGO SYP", div: "MEDK", pack: "100ML", batch: "AL26040", exp: "9/27", hsn: "30049099", oldMrp: 134, mrp: 112, baseRate: 17, rate: 56, igst: 5, qty: 20, free: 0 },
    ...
    { id: "M43", name: "THECORT-6 TABS. ALU ALU", div: "MEDK", pack: "10X10", batch: "AT-2584", exp: "12/27", hsn: "30049085", oldMrp: 130, mrp: 1396, baseRate: 130, rate: 400, igst: 5, qty: 3, free: 0 },
...
// CACHE KEY v26 - Bug-Free Schema with Smart JSON Import
if (!localStorage.getItem("shreya_db_stock_v26")) localStorage.setItem("shreya_db_stock_v26", JSON.stringify(exactMasterData));
if (!localStorage.getItem("shreya_db_sales_v26")) localStorage.setItem("shreya_db_sales_v26", JSON.stringify([]));
if (!localStorage.getItem("shreya_db_counter_v26")) localStorage.setItem("shreya_db_counter_v26", "1");

let dbStock = JSON.parse(localStorage.getItem("shreya_db_stock_v26"));
let dbSales = JSON.parse(localStorage.getItem("shreya_db_sales_v26"));
let invCounter = parseInt(localStorage.getItem("shreya_db_counter_v26"));
let currentCart = [];
...
function renderCheatSheet() {
    ...
    dbStock.forEach(item => {
        if(item.mrp === 0 || item.rate === 0) return; // Skip empty rows
        
        if(item.name.toLowerCase().includes(searchTerm)) {
            // ((MRP - Target Rate) / MRP) * 100
...
function saveNewMedicine() {
    ...
    if(editId) {
        ...
        if(idx !== -1) {
            ...
            dbStock[idx].rate = rate; // Target Rate
    ...
    localStorage.setItem("shreya_db_stock_v26", JSON.stringify(dbStock));
...
function autoFillDetails() {
    ...
    if(item) {
        ...
        safeSet("billRealRate", item.rate); // Shows Target Rate on POS Screen
        safeSet("billSellRate", item.rate); // DEFAULT BILLING RATE IS NOW TARGET RATE FROM PDF
    ...
    currentCart.forEach((item, index) => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>${item.billFree}</td>
                <td>₹${formatAmt(item.sellRate)}</td> <!-- POS MRP Column -->
                <td style="color:var(--secondary);">${item.discount}%</td>
                <td>${item.igst}%</td>
                <td style="color:var(--primary); font-weight:800;">₹${formatAmt(taxableValue)}</td> <!-- POS Rate Column -->
...
function generateBill() {
    ...
    currentCart.forEach((item, i) => {
        ...
        pBody.innerHTML += `
            <tr>
                ...
                <td>${item.billQty}</td>
                <td>${item.billFree}</td>
                <td>${formatAmt(item.sellRate)}</td> <!-- Representing MRP Header -->
                <td>${item.discount}%</td>
                <td>${formatAmt(taxableValue)}</td> <!-- Representing Rate Header -->
                <td>${item.igst}%</td>
    ...
    localStorage.setItem("shreya_db_stock_v26", JSON.stringify(dbStock));
    localStorage.setItem("shreya_db_sales_v26", JSON.stringify(dbSales));
    localStorage.setItem("shreya_db_counter_v26", invCounter);
...
// ================= CLOUD BACKUP (SMART RESTORE BUG FIX) =================
function exportData() {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (dataObj.stock && dataObj.sales) {
                ...
                let uploadedStock = dataObj.stock;
                let freshStock = JSON.parse(JSON.stringify(exactMasterData)); // This template preserves our perfect new Rates
                
                // SMART MERGE: Only pull Quantities from the old backup
                uploadedStock.forEach(upItem => {
                    let match = freshStock.find(m => m.id === upItem.id);
                    if(match) {
                        match.qty = upItem.qty || 0;
                        match.free = upItem.free || 0;
                        // We intentionally DO NOT import upItem.baseRate or upItem.rate to prevent old bugs from returning!
                    } else {
                        // If it's a completely new custom medicine added by the user
                        if(upItem.baseRate === undefined) upItem.baseRate = upItem.rate || 0;
                        freshStock.push(upItem);
                    }
                });

                dbStock = freshStock;
                dbSales = dataObj.sales;
                invCounter = dataObj.counter || invCounter;
                
                localStorage.setItem("shreya_db_stock_v26", JSON.stringify(dbStock));
                localStorage.setItem("shreya_db_sales_v26", JSON.stringify(dbSales));
                localStorage.setItem("shreya_db_counter_v26", invCounter);
                
                updateUI(); renderCheatSheet(); showToast("Smart Restore Successful!");
...
function renderReports() {
    ...
    let manualIgst = localStorage.getItem("shreya_igst_final_v26");
...
function editIgstTotal() {
    let currentManual = localStorage.getItem("shreya_igst_final_v26");
    let input = prompt("Enter manual IGST collected amount (Leave blank to auto-calculate):", currentManual || "");
    if(input !== null) {
        if(input.trim() === "") {
            localStorage.removeItem("shreya_igst_final_v26"); showToast("IGST set to Auto!");
        } else {
            localStorage.setItem("shreya_igst_final_v26", parseFloat(input)); showToast("IGST Updated!");
        }
        renderReports();
    }
}